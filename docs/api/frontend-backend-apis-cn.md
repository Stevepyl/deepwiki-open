...
| `ws://<backend>/ws/chat` | WebSocket | `/ws/chat` | 浏览器直连 WebSocket | 流式传输旧版原始文本 chat、slides 和 workshop 内容。 |
| `/api/chat/stream` | `POST` | `/chat/completions/stream` | Next route proxy | `/ws/chat` 的 HTTP 流式回退方案。 |
| `ws://<backend>/ws/agent-chat` | WebSocket | `/ws/agent-chat` | 浏览器直连 WebSocket | 为后续 Ask UI 流式传输结构化 agent chat 事件。 |
| `/api/chat/agent-stream` | `POST` | `/chat/agent-stream` | Next route proxy | 结构化 agent chat 事件的 HTTP NDJSON 回退方案。 |
| `ws://<backend>/ws/agent-wiki` | WebSocket | `/ws/agent-wiki` | 浏览器直连 WebSocket | 通过结构化两阶段 agent 事件生成 wiki cache。 |

### 后端已注册但当前前端未调用的 API

| 后端 URL | 方法 | 用途 |
|---|---:|---|
| `/agent/info` | `GET` | 列出可用于聊天的 agent 配置（`wiki`、`explore`、`deep-research`），供后续 agent picker 使用。 |
...
由 `/ws/agent-chat` 和 `/api/chat/agent-stream` 使用。
...
- 默认 `agent_name` 是 `explore`，使用只读工具和较小的 step budget。
- 后端会拒绝未知 agent，并要求至少有一条可用 message，且最后一条的 role 为 `user`。
- 请求会先转换为内部 `AgentMessage`，但浏览器侧保持旧的 `{ role, content }` chat 数组形态。

### `AgentChatEvent`

两个 agent chat transport 都使用该事件形态。WebSocket 中每条消息是一个 JSON 对象；HTTP 中每一行 NDJSON 是一个 JSON 对象。
...
`api/agent/stream_events.py` 还定义了 wiki 专用和前向兼容事件。当前前端的 agent-chat 类型文件只包含这五类聊天相关事件。
...
- chat-style prompt 的旧版原始文本流式端点。
- 仍用于 slides 和 workshop 生成。
- 当前首次 wiki cache 生成改用 `/ws/agent-wiki`。
...
- 为重新设计后的 Ask 页面提供结构化 agent chat 传输。
- 暴露 agent reasoning 和 tool-use 事件；这些信息无法通过 `/ws/chat` 的原始文本可靠表达。
- 由 `src/utils/websocketClient.ts#createAgentChatWebSocket` 使用。

请求：

客户端打开 socket，等待 `onopen`，然后发送一个 `AgentChatRequest` JSON 对象：
...
- 服务端发送带有判别字段 `type` 的结构化 JSON 事件。
- `text_delta` 追加到 assistant message。
- `tool_call_start` 和 `tool_call_end` 更新工具/推理 UI 状态。
- `error` 后面可以跟一个 `finish_reason: "error"` 的终止 `finish` 事件。
- 正常会话以且仅以一个 `finish` 事件结束。

校验与错误：

| 条件 | 事件序列 |
|---|---|
| 未知 `agent_name` | `code: "unknown_agent"` 的 `error`，随后 `finish`。 |
| `messages` 为空 | `code: "empty_messages"` 的 `error`，随后 `finish`。 |
| 最后一条消息不是 `role: "user"` | `code: "last_message_not_user"` 的 `error`，随后 `finish`。 |
| 仓库 clone/download 失败 | `code: "clone_failed"` 的 `error`，随后 `finish`。 |
| Provider 初始化失败 | `code: "provider_error"` 的 `error`，随后 `finish`。 |

实现说明：

- 该 WebSocket 为浏览器直连后端，不经过 `next.config.ts` 代理。
- 它与旧 chat helper 一样使用 `SERVER_BASE_URL.replace(/^http/, "ws")` 约定。
- PLAN-007 只添加了 connector utility。当前 `src/app/[owner]/[repo]/ask/page.tsx` 仍使用 `/ws/chat`；PLAN-005 负责把重新设计的 Ask 页面接入这个结构化流。
...
- `/ws/agent-chat` 的 HTTP 回退方案。
- 通过 line-delimited JSON 保持相同的结构化 agent event 形态。
- 由 `src/utils/agentChatStream.ts#streamAgentChatHttp` 使用。

请求：

与 `/ws/agent-chat` 的 `AgentChatRequest` 结构相同。

响应：

- Next route handler 返回 `ReadableStream`。
- 后端返回 `StreamingResponse(..., media_type="application/x-ndjson")`。
- 每个非空行都是一个序列化后的 `AgentChatEvent`。
- Next 代理会设置 `Cache-Control: no-cache, no-transform`，并在存在时转发后端的 `Content-Type`。

实现说明：

- 这不是 SSE。客户端应按 `\n` 拆分，并将每行解析为 JSON。
- WebSocket 和 HTTP agent transport 发出同样的事件对象，所以重新设计的 Ask 页面可以复用同一个 `onEvent` reducer。
...
- 已在 FastAPI 中注册。
- 由 `src/utils/wikiGeneration.ts#generateWikiCache` 用于首次和刷新时的 wiki cache 生成。
- Slides 与 workshop 生成仍使用旧版原始文本 `/ws/chat` helper。
...
- 当前前端 cache 构建逻辑会消费 `wiki_structure_ready`、`wiki_page_done`、`wiki_structure_error`、`wiki_page_error`、`error` 和 `finish`；工具与文本事件不参与 cache 构建。
...
仓库 wiki 页面
  ...
  │    ├─ wiki_structure_ready → 构建 wiki_structure
  │    └─ wiki_page_done per page → 构建 generated_pages
  │
  └─ POST /api/wiki_cache
       └─ 持久化生成后的 wiki
```

该路径不使用原始文本 `POST /api/chat/stream` 回退。`wiki_structure_error`、`wiki_page_error`、通用 `error`、socket 失败或超时会让加载页进入失败状态，并把错误信息展示给用户。
...
PLAN-007 还为即将进行的 PLAN-005 Ask 改造添加了结构化 agent-chat connector 路径：

```text
重新设计后的 Ask 页面
  ├─ WebSocket /ws/agent-chat
  │    └─ 流式返回 AgentChatEvent JSON 对象
  └─ POST /api/chat/agent-stream
       └─ 使用相同事件形态的 HTTP NDJSON 回退
```

在该路径中，`text_delta` 填充 assistant message，`tool_call_start` / `tool_call_end` 驱动推理或工具时间线。当前 `src/` 中的页面级 UI 尚未接入。
...
REST 路由可以通过 Next 以同源方式访问，但 `/ws/chat`、`/ws/agent-chat` 和 `/ws/agent-wiki` 都是直连后端主机。生产部署必须将 FastAPI WebSocket 端点暴露给浏览器，或额外添加一个 WebSocket 代理。
...
`/ws/agent-chat` 和 `/ws/agent-wiki` 不同：错误是结构化事件对象，随后会有终止 `finish` 事件或按具体路由关闭 socket。不要把原始文本错误解析逻辑复用到 agent 路径。
