...
def _collect_ws_events(payload):
...
def test_agent_chat_websocket_happy_path(monkeypatch):
    _patch_happy_path(monkeypatch)

    events = _collect_ws_events(_payload())
...
def test_agent_chat_websocket_unknown_agent():
    events = _collect_ws_events(_payload(agent_name="bogus"))
...
def test_agent_chat_websocket_invalid_payload():
    events = _collect_ws_events({"messages": [{"role": "user", "content": "hi"}]})
...
def test_agent_chat_websocket_clone_failure(monkeypatch):
...
def test_agent_chat_websocket_provider_init_failure(monkeypatch):
    monkeypatch.setattr(chat_handler, "download_repo", lambda *args: None)
...
def test_agent_chat_websocket_empty_messages():
    events = _collect_ws_events(_payload(messages=[]))
...
def test_agent_chat_websocket_last_message_not_user():
    events = _collect_ws_events(
        _payload(messages=[{"role": "assistant", "content": "hello"}])
    )
