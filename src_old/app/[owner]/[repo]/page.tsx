...
import { FaBitbucket, FaBookOpen, FaDownload, FaExclamationTriangle, FaFileExport, FaFolder, FaGithub, FaGitlab, FaHome, FaSync } from 'react-icons/fa';
...
4. Return ONLY valid XML with the structure specified above, with no markdown code block delimiters`
  ...
  const [askDeepResearch, setAskDeepResearch] = useState(false);
  ...
  const handleAskSubmit = async (askedQuestion: string, options: AskSubmitOptions) => {
    ...
    nextParams.set('question', askedQuestion);
    nextParams.set('deep_research', options.deepResearch ? 'true' : 'false');
    if (!nextParams.get('language')) {
      nextParams.set('language', language);
    }
    router.push(`/${encodeURIComponent(owner)}/${encodeURIComponent(repo)}/ask?${nextParams.toString()}`);
  };
  ...
  return (
    <div className="h-screen bg-white p-4 md:p-8 flex flex-col">
      ...
      <footer className="max-w-[90%] xl:max-w-[1400px] mx-auto mt-8 mb-[124px] md:mb-[136px] flex flex-col gap-4 w-full">
        <div className="flex justify-between items-center gap-4 text-center text-[var(--muted)] text-sm h-fit w-full bg-[var(--card-bg)] rounded-lg p-3 shadow-sm border border-[var(--border-color)]">
          <p className="flex-1 font-serif">
            {messages.footer?.copyright || 'OpsWiki - Generate Wiki from GitHub/Gitlab/Bitbucket repositories'}
          </p>
          <ThemeToggle />
        </div>
      </footer>
      ...
      {!isLoading && wikiStructure && (
        <div className="fixed left-0 right-0 bottom-0 z-40 border-t border-gray-200 bg-white/95 backdrop-blur">
          <div className="max-w-[90%] xl:max-w-[1400px] mx-auto py-2.5">
            <AskComposer
              compact
              value={askQuestionInput}
              onValueChange={setAskQuestionInput}
              deepResearch={askDeepResearch}
              onDeepResearchChange={setAskDeepResearch}
              onSubmit={async (askedQuestion, options) => {
                setAskQuestionInput('');
                await handleAskSubmit(askedQuestion, options);
              }}
              className="border-transparent shadow-none p-0 bg-transparent"
            />
          </div>
        </div>
      )}
