        ...
        return (
          <section className={done ? "opacity-45" : ""} id={step.id} key={step.id}>
            <div className="mb-3 mt-12 flex items-baseline gap-3 first:mt-0">
              <span className="shrink-0 font-[var(--font-mono)] text-[10.5px] font-medium uppercase tracking-normal text-[var(--accent)]">
                {done ? `${String(index + 1).padStart(2, "0")} - done` : `Step ${String(index + 1).padStart(2, "0")}`}
              </span>
              <h2 className="m-0 font-[var(--font-serif)] text-2xl font-semibold leading-[1.2] tracking-normal text-[var(--ink-primary)]">
                {step.title}
              </h2>
            </div>
