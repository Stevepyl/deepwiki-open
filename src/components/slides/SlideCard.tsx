...
export function SlideCard({ slide, stub = false }: SlideCardProps) {
  ...
  if (slide.variant === "divider") {
    return (
      <div className={`${baseClass} items-center justify-center gap-5 text-center`}>
        <div className="font-mono text-[11px] uppercase tracking-normal text-[var(--ink-muted)]">
          {slide.eyebrow}
        </div>
        <h1 className="m-0 text-balance font-serif text-[44px] font-semibold leading-[1.1] tracking-normal text-[var(--ink-primary)]">
          {slide.title}
        </h1>
        {slide.subtitle && (
          <p className="m-0 font-serif text-[19px] italic text-[var(--ink-secondary)]">{slide.subtitle}</p>
        )}
      </div>
    );
  }
  ...
  return (
    <div className={`${baseClass} gap-6 overflow-hidden`}>
      {slide.eyebrow && (
        <div className="flex items-center gap-2 font-sans text-[10.5px] font-medium uppercase tracking-normal text-[var(--accent)] before:h-px before:w-4 before:bg-[var(--accent-line)] before:content-['']">
          {slide.eyebrow}
        </div>
      )}
      <h2 className="m-0 font-serif text-[32px] font-semibold leading-[1.15] tracking-normal text-[var(--ink-primary)]">
        {slide.title}
      </h2>
