...
export function WikiArticle({ page, sectionTitle }: WikiArticleProps) {
  ...
  return (
    <article className="min-w-0 pb-6 text-[var(--ink-primary)]">
      <div className="mb-3 flex items-center gap-2.5 font-[var(--font-sans)] text-[11px] font-medium uppercase tracking-normal text-[var(--accent)] before:h-px before:w-5 before:bg-[var(--accent-line)] before:content-['']">
        <span>{sectionTitle}</span>
      </div>
      <h1 className="m-0 mb-5 font-[var(--font-serif)] text-[40px] font-semibold leading-[1.15] tracking-normal text-[var(--ink-primary)]">
        {page.title}
      </h1>
      {lede && (
        <p className="mb-10 max-w-[640px] font-[var(--font-serif)] text-[19px] italic leading-[1.55] text-[var(--ink-secondary)]">
          {lede}
        </p>
      )}
      <div className="mb-10 flex flex-wrap gap-x-5 gap-y-2 border-y border-[var(--hairline)] py-3.5 font-[var(--font-sans)] text-[11.5px] tracking-normal text-[var(--ink-muted)]">
        <span>
