...
| [PLAN-002 - OpsWiki Frontend Refinement Overview](PLAN-002-frontend-refinement-overview.md) | `proposed` | `in-progress` | PLAN-003, PLAN-004, and PLAN-005 are implemented in `src`; PLAN-006 is still not started, and the legacy `src_old/` frontend remains untouched per the overview non-goal. | Cross-cutting plan; sub-tasks live in PLAN-003 through PLAN-006. |
...
| [PLAN-006 - Wiki Workshop Slides and Loading](PLAN-006-wiki-family.md) | `proposed` | `not-started` | Plan frontmatter is `status: proposed`; `src_old/app/[owner]/[repo]/page.tsx` is 2267 lines and `src_old/components/WikiTreeView.tsx` is still the TOC; no `src_old/components/generation/` folder exists. Now carries a Tailwind-first styling rule shared with PLAN-004 and PLAN-005. | Depends on PLAN-003; keeps wiki-family generation on raw-text `/ws/chat`. |
...
| PLAN-006 | Rewrite `src_old/app/[owner]/[repo]/page.tsx` wiki route | `not-started` | Under 250 lines target. |
| PLAN-006 | Rewrite `src_old/app/[owner]/[repo]/workshop/page.tsx` | `not-started` | Under 200 lines target. |
| PLAN-006 | Rewrite `src_old/app/[owner]/[repo]/slides/page.tsx` | `not-started` | Under 200 lines target. |
