...
| [PLAN-002 - OpsWiki Frontend Refinement Overview](PLAN-002-frontend-refinement-overview.md) | `implemented` | `implemented` | PLAN-003, PLAN-004, PLAN-005, and PLAN-006 are implemented in `src`; the legacy `src_old/` frontend remains untouched per the overview non-goal; `bun run lint` and `bun run build` pass. | Cross-cutting plan; sub-tasks live in PLAN-003 through PLAN-006. |
| [PLAN-003 - Foundation Design Tokens and App Shell](PLAN-003-foundation.md) | `implemented` | `implemented` | Plan frontmatter is `status: implemented`; `src/app/globals.css` contains only Tailwind imports, Paper and Ink tokens, and base element defaults; `src/app/layout.tsx` mounts `LanguageProvider` and `SettingsProvider` without `ThemeProvider`; `src/components/shell/` contains Tailwind-first AppShell, Sidebar, Topbar, Composer, Wordmark, SettingsPanel, and small chrome primitives; shared renderers/connectors/types are copied into `src`; `src/messages/en.ts` centralizes shell copy; `bun run lint` and `bun run build` pass with existing warnings only. | Blocks PLAN-004, PLAN-005, PLAN-006; owns copying shared chat/agent-chat connector files into `src`. |
| [PLAN-004 - Welcome and Projects Directory](PLAN-004-welcome-and-projects.md) | `implemented` | `implemented` | Plan frontmatter is `status: implemented`; `src/app/page.tsx` is 111 lines and renders the OpsWiki welcome route; `src/app/projects/page.tsx` is 117 lines and renders the project directory; `src/components/welcome/*`, `src/components/projects/*`, `src/hooks/useProjectsDirectory.ts`, and `src/app/api/wiki/projects/route.ts` implement the supporting UI/data pieces; `bun run lint` and `bun run build` pass. | Depends on PLAN-003. |
...
| PLAN-003 | Token rewrite of `globals.css` | `implemented` | `src/app/globals.css` registers Paper and Ink tokens plus base element defaults only; wordmark, shell/composer chrome, SettingsPanel chrome, and markdown styles now live in Tailwind utility classes inside `src` components. |
| PLAN-003 | Build shared shell components under `src/components/shell/` | `implemented` | Includes `<AppShell>`, `<Sidebar>`, `<Topbar>`, `<Composer>`, `<Wordmark>`, `<SettingsPanel>`, `<IconButton>`, `<Switcher>`, `<Eyebrow>`, and `<DayDivider>`. |
| PLAN-003 | Copy shared legacy chat and agent-chat connector files into `src` | `implemented` | Preserves `createChatWebSocket`, `createAgentChatWebSocket`, `streamAgentChatHttp`, and `AgentChatEvent`/`AgentChatRequest` types with self-contained `src` imports. |
...
| PLAN-003 | Preserve legacy `src_old/` while adding `src` foundation | `implemented` | Existing `src_old/` pages and components are untouched, matching PLAN-002 and PLAN-003 scope. |
| PLAN-004 | Rewrite `src/app/page.tsx` welcome route | `implemented` | 111 lines; renders the wordmark, subtitle, scratch input, example grid, `/projects` link, and welcome keyboard hint. |
| PLAN-004 | Create `src/app/projects/page.tsx` and project directory components | `implemented` | 117-line route with alpha rail, filter input, grid/list persistence, grouped cards, conversation counts, and delete flow through `/api/wiki/projects`. |
| PLAN-005 | Rewrite `src/app/[owner]/[repo]/ask/page.tsx` | `implemented` | 197-line route; uses `AgentChatRequest` with default `agent_name: "explore"`, deep-research mapping, and advanced `wiki` selection. |
| PLAN-005 | Build `<ChatStream>`, `<Message>`, `<Citation>`, and `<ToolEvent>` under `src/components/chat/` | `implemented` | Reuses `<Markdown>` and renders structured `tool_call_start` / `tool_call_end` events without importing from `src_old/`. |
| PLAN-005 | Wire WebSocket + HTTP agent-chat transports | `implemented` | Primary `/ws/agent-chat`; fallback `/api/chat/agent-stream`; both feed the same `AgentChatEvent` reducer. |
| PLAN-006 | Rewrite `src/app/[owner]/[repo]/page.tsx` wiki route | `implemented` | 142-line route renders cached wiki content, handles `?status=generating`, redirects composer submissions to Ask, exposes slides and export actions, and does not import from `src_old/`. |
| PLAN-006 | Rewrite `src/app/[owner]/[repo]/workshop/page.tsx` | `implemented` | 121-line route renders the workshop progress rail and article, seeds localStorage from cached wiki content, and background-generates workshop steps through raw `/ws/chat`. |
| PLAN-006 | Rewrite `src/app/[owner]/[repo]/slides/page.tsx` | `implemented` | 103-line route renders the slides presenter with previous/next stubs, dots, counter, fullscreen, and keyboard controls. |
| PLAN-006 | Build `<GenerationLoader>` and `useGenerationPhases` hook | `implemented` | `GenerationLoader` is 114 lines; `useGenerationPhases.ts` is 99 lines and performs best-effort phase parsing over raw text, not `AgentChatEvent`. |
...
| PLAN-007 | Frontend types `src_old/types/agentChat.ts` | `implemented` | Discriminated union covers chat-relevant agent events and request payloads. |
